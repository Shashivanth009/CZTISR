...
export const SurveillanceRecon = ({ token }: Props) => {
        ...
        const fetchData = async () => {
            try {
                const [sResp, rResp] = await Promise.all([
                    fetch(`${config.gatewayUrl}/api/surveillance`),
                    fetch(`${config.gatewayUrl}/api/reconnaissance`),
                ]);
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Surveillance Zones */}
            <GlassExpand title="SURVEILLANCE ZONES">
            ...
            <GlassExpand title="RECONNAISSANCE ASSETS">
                <div className="glass-panel p-5 rounded">
                    <h3 className="text-sm font-bold text-white mb-4 flex items-center gap-2">
