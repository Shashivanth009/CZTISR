...
export const Communications = ({ token, user }: Props) => {
        ...
        const fetchData = async () => {
            try {
                const [cResp, mResp] = await Promise.all([
                    fetch('http://localhost:8020/api/communications'),
                    fetch('http://localhost:8020/api/communications/messages'),
                ]);
    ...
    const sendMessage = async () => {
        ...
        try {
            await fetch('http://localhost:8020/api/communications/send', {
                method: 'POST',
